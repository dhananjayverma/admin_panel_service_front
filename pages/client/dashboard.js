    ...
    <ClientLayout>
      ...
      {loading ? <LoadingSpinner /> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 20 }}>
            <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>Total Campaigns</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: '#1e293b', marginTop: 4 }}>{data?.overview?.totalCampaigns ?? 0}</p>
          </div>
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 20 }}>
            <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>Messages Sent</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: '#1e293b', marginTop: 4 }}>{data?.overview?.totalSent ?? 0}</p>
          </div>
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 20 }}>
            <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>Failed</p>
            <p style={{ fontSize: 24, fontWeight: 700, color: '#1e293b', marginTop: 4 }}>{data?.overview?.totalFailed ?? 0}</p>
          </div>
        </div>
      )}
    </ClientLayout>
