...
export default function AdminDpCampaign() {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    api.campaigns.list().then((r) => setCampaigns((r.campaigns || []).filter((c) => c.type === 'dp'))).catch(() => setCampaigns([])).finally(() => setLoading(false));
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f172a', margin: '0 0 8px 0' }}>DP Campaign</h1>
      <p style={{ fontSize: 14, color: '#64748b', marginBottom: 24 }}>Whatsapp Bulk / Campaigns / DP Campaign</p>
      {loading ? <LoadingSpinner /> : campaigns.length === 0 ? <EmptyState message="No DP campaigns yet." /> : (
