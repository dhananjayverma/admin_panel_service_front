...
export default function AdminButtonCampaign() {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    api.campaigns.list().then((r) => setCampaigns((r.campaigns || []).filter((c) => c.type === 'button'))).catch(() => setCampaigns([])).finally(() => setLoading(false));
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0f172a', margin: '0 0 8px 0' }}>Button Campaign</h1>
      <p style={{ fontSize: 14, color: '#64748b', marginBottom: 24 }}>Whatsapp Bulk / Campaigns / Button Campaign</p>
      {loading ? <LoadingSpinner /> : campaigns.length === 0 ? <EmptyState message="No button campaigns yet." /> : (
